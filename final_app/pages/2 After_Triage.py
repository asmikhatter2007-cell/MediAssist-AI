...
/* ================= GLOBAL / HEADER FIX ================= */
/* Streamlit's top header bar is white/transparent by default — this was
   the visible gap. Force it to match the app background. */
...
section[data-testid="stSidebar"]{
    background: linear-gradient(180deg, #0D1226 0%, #0C1024 100%);
    border-right: 1px solid rgba(255,255,255,0.07);
}
section[data-testid="stSidebar"] .block-container{ padding-top: 2rem; }
/* Sidebar collapses on narrow windows by default — the little ">" arrow
   top-left re-opens it. Making the collapsed-state arrow easier to see: */
...
/* ================= SELECT BOX — broadened to catch every nested div ================= */
...
/* Dropdown menu — renders in a separate floating layer */
...
.stButton>button{
    ...
    font-weight:800;
    border:none;
    box-shadow: 0 14px 34px rgba(129,140,248,0.30);
    transition: all 0.25s ease;
}
.stButton>button:hover{
    background-position: right center;
    transform: translateY(-2px);
    box-shadow: 0 18px 42px rgba(244,114,182,0.35);
    color:#0C1024;
}
...
st.caption("AI-powered Clinical Decision Support for Emergency Departments")
...
if predict:
    ...
    with st.spinner("Generating predictions..."):
        ...
        payload = {
            ...
            "floor_or_chair_care": FLOOR_OR_CHAIR_CARE,
            "nurse_patient_ratio": NURSE_PATIENT_RATIO,
            "doctor_patient_ratio": DOCTOR_PATIENT_RATIO,
        ...
        try:
            ...
            response = requests.post(
                "http://127.0.0.1:8000/predict_waittime",
                json=payload
            )
            ...
            response = requests.post(
                "http://127.0.0.1:8000/predict_doctor_wait",
                json=doctor_payload
            )
...
if predict:
    ...
    with st.container(border=True):
        ...
        if triage_category == "very_urgent":
            risk_score += 2

        elif triage_category == "urgent":
            risk_score += 1

        elif triage_category == "emergency":
            risk_score += 2
        ...
        if DOCTOR_PATIENT_RATIO >= 12:
            risk_score += 1

        risk_score = min(risk_score, 7)
        ...
        if risk_score >= 5:
            ...
            risk_label = "MODERATE"
            risk_color = "#FBBF6E"
            recommendation = "Prioritize assessment and continue close monitoring."
