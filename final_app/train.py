...
df = df[df['wait_triage_min'] <= 500].copy()
print(f"After removing outliers: {df.shape}")

print("\nWait time stats:")
print(df['wait_triage_min'].describe())
...
df = df.drop(columns=[
    'id',
    'wait_doctor_min',          # happens AFTER triage wait
...
joblib.dump(best_pipeline,"wait_time_model.pkl")

print(f"\n Best model ({best_name}) saved as wait_time_model")

print(df[["chief_complaint",
"wait_triage_min",
"ed_overcrowded",
"bed_available"]].head(10))
# =====================================================
